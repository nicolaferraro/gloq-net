...
public class LocalMatching extends IncrementalExecutable<Set<Set<Matching>>, Set<Matching>> {
	...
	protected void execute() throws Exception {
		...
		do {
			...
			GraphCleaner cleaner = new GraphCleaner(graphs, primaryDict);
			...
			FindSubgraph finder = new FindSubgraph(primaryDict, secondaryDict, graphs, depth);
				...
				if(isNew) {
					...
					for(int i=0; i<graphs.size(); i++) {
						for(int j=i+1; j<graphs.size(); j++) {
							...
							primaryDict.removeEntry(n1.getName(), n2.getName());
						}
					}
				}
